...
!================================================
           do i=1,nx
              do j=1,ny
                 umean(i,j)=umean(i,j)+u(i,j,ianc)
                 vmean(i,j)=vmean(i,j)+v(i,j,ianc)
                 uabs(i,j)=uabs(i,j)+abs(u(i,j,ianc))
                 vabs(i,j)=vabs(i,j)+abs(v(i,j,ianc))
                 tauu(i,j)=tauu(i,j)+abs(dyu(i,j,ianc))
                 tauv(i,j)=tauv(i,j)+abs(dxv(i,j,ianc))
                 pmean(i,j)=pmean(i,j)+p(i,j,ianc)
              enddo
           enddo
           do i=1,nx
              do j=1,ny
                 umean(i,j)=umean(i,j)/float(ite)
                 vmean(i,j)=vmean(i,j)/float(ite)
                 uabs(i,j)=uabs(i,j)/float(ite)
                 vabs(i,j)=vabs(i,j)/float(ite)
                 tauu(i,j)=tauu(i,j)/float(ite)
                 tauv(i,j)=tauv(i,j)/float(ite)
                 pmean(i,j)=pmean(i,j)/float(ite)
...
c              write(400,*)
           ...
           call stockage(vmean,xmesh,ymesh,nx,ny,ite,past,'Vm')
           call stockage(tauu,xmesh,ymesh,nx,ny,ite,past,'tauv')
           call stockage(tauv,xmesh,ymesh,nx,ny,ite,past,'tauu')
           call stockage(pmean,xmesh,ymesh,nx,ny,ite,past,'Pm')
           call stockage(uabs,xmesh,ymesh,nx,ny,ite,past,'Uabs')
           call stockage(vabs,xmesh,ymesh,nx,ny,ite,past,'Vabs')
!=================================================================
