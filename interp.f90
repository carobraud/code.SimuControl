...
!-> controle
  ...
  call controle(nxi,nyi,xi,yi(1,:),ui,vi,nxc,xc,uc,vc,xca,xcb,mask)

  !print*, "EXIT FORTRAN"
  !print*,nxc,xca,xcb,xc(1:nxc),uc(1:nxc),vc(1:nxc)
...
!     print*,'index 1=',ica,'index 2=',icb
  enddo

 ! print*,'MESH: ',xca,xmesh(ica,1),xc(1),xcb,xmesh(icb,1),xc(nxc)
  ...
  do i=ica,icb
     ...
     v(i,1)=vc(idx(1))+(xmesh(i,1)-xc(idx(1)))*&
          (vc(idx(1)+1)-vc(idx(1)))*&
          (xc(idx(1)+1)-xc(idx(1)))
!     print*,"Fortran:mesh +velocity",xmesh(i,1),xc(idx(1)),xc(idx(1)+1),vc(idx(1)),v(i,1),vc(idx(1)+1)
  enddo
