...
c      write(*,*) ite
c      if ((ite.ge.icontb).and.(ite.le.icontc)) then
!     if (ite.ge.icontb) then
c     write(*,*) 'ITE=',ite
c         ampl=0.1d0
c         do i=1,5
c            xfc(i)=1.d0
c            u(nvit+5+i,1,iact)=ampl*xfc(i) !*xfc(i)/sqrt(2.d0)
c            v(nvit+5+i,1,iact)=ampl*xfc(i) !*xfc(i)/sqrt(2.d0)
c         enddo
c      else if ((ite.ge.icontc).and.(ite.le.icontd)) then
c         do i=1,5
c            xfc(i)=1.d0
c            v(nvit+56+i,1,iact)=-0.1*xfc(i)
c         enddo
c      write(210,*) xite,u(nvit+57,1,iact),v(nvit+57,1,iact)
c      else
c         do i=1,5
c            u(nvit+5+i,1,iact)=0.d0
c            v(nvit+5+i,1,iact)=0.d0
c            v(nvit+56+i,1,iact)=0.d0
c         enddo
c      end if
...
!================================================
c        ipasPIV=500
c        if (ite-ipasPIV*(ite/ipasPIV).eq.0.and.ite>0) then
           write(num,'(i6)')100000+int(ite*past)
!     write(*,*) num
!     write(nom,'(a5,a6,a3)')'Uinst',num(1:6),'.nc'

           call interp(nx,ny,xmesh,ymesh,u(1,1,ianc),v(1,1,ianc),wz(1,1)
     &          ,num)
