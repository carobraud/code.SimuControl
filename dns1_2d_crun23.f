...
! PARAMETRE STOCKAGE
      ...
      dimension umean(nx,ny),tmpum(nx,ny)
      dimension pmean(nx,ny),tmpp(nx,ny)

! PARAMETRE STOCKAGE NETCDF
      character pathresults*100,fileout*100
      character(6) num
      dimension unc(nx,ny,1,1)
      dimension vnc(nx,ny,1,1)
      dimension wnc(nx,ny,1,1)
      dimension t(1),x(nx),z(1)
...
c      if (ite.ge.icontb) then
c      write(*,*) 'ITE=',ite
         ampl=0.05d0
         do i=1,5
            xfc(i)=1
            u(nvit+5+i,1,iact)=ampl*xfc(i) !*xfc(i)/sqrt(2.d0)
            v(nvit+5+i,1,iact)=ampl*xfc(i) !*xfc(i)/sqrt(2.d0)
         enddo
         else
c         do i=1,5
c            u(nvit+5+i,1,iact)=0.d0
c            v(nvit+5+i,1,iact)=0.d0
c         enddo
      end if
      write(210,*) xite,u(nvit+10-1,1,iact),v(nvit+10-1,1,iact)
...
!      nu?=nvit+5 !(x,y)=(25.8,1.957)
      ...
      nu11=216 !(x,y)=(43 7.45576771E-04)
      nu12=241 !(x,y)=(48,0.000357)
      nu13=326 !(x,y)=(65,0)
      nu14=401 !(x,y)=(80,0)
      nu15=501 !(x,y)=(100,0)
      nu16=601 !(x,y)=(120,0)
     ...
     &dyu(nu11,2,iact),dxv(nu11,2,iact),p(nu11,2,iact)
      write(312,108)xite,u(nu12,2,iact),v(nu12,2,iact),
     &dyu(nu12,2,iact),dxv(nu12,2,iact),p(nu12,2,iact)
      write(313,108)xite,u(nu13,2,iact),v(nu13,2,iact),
     &dyu(nu13,2,iact),dxv(nu13,2,iact),p(nu13,2,iact)
      write(314,108)xite,u(nu14,2,iact),v(nu14,2,iact),
     &dyu(nu14,2,iact),dxv(nu14,2,iact),p(nu14,2,iact)
      write(315,108)xite,u(nu15,2,iact),v(nu15,2,iact),
     &dyu(nu15,2,iact),dxv(nu15,2,iact),p(nu15,2,iact)
      write(316,108)xite,u(nu16,2,iact),v(nu16,2,iact),
     &dyu(nu16,2,iact),dxv(nu16,2,iact),p(nu16,2,iact)
...
!
! =====================================================================
! =            ENREGISTREMENT QUELQUES GRANDEURS
! =====================================================================
! 
!     ipas2=5000
        ipas2=500
...
!     calcul conv Um, Pm
!           ipasLF=500
!           if (ite-ipasLF*(ite/ipasLF).eq.0.and.ite>0) then
              do i=1,nx
                 do j=1,ny
                    pmean(i,j)=tmpp(i,j)/float(ipas2)
                    umean(i,j)=tmpum(i,j)/float(ipas2)
                 enddo
              enddo

!Calcul lf: USING changement signe of Um avec temps
              do i=nvit-20,nx-10
           ...
           call stockage(umean,xmesh,ymesh,nx,ny,ite,past,'Um') 
           call stockage(pmean,xmesh,ymesh,nx,ny,ite,past,'Pm') 
        endif

!================================================
!     XMESH,YMESH vers sous domaine cartesien (XI,YI,XA,XB,YA,YB) 
!     + STOCKAGE DE CE SOUS DOMAINE VERS FICHIER NETCDF
!================================================
        ipasPIV=500
        if (ite-ipasPIV*(ite/ipasPIV).eq.0.and.ite>0) then
           write(num,'(i6)')100000+int(ite*past)
!     write(*,*) num
!     write(nom,'(a5,a6,a3)')'Uinst',num(1:6),'.nc'
           
           call interp(nx,ny,xmesh,ymesh,u(1,1,ianc),v(1,1,ianc),num)
        endif
!        stop

!================================================
!  TODO: SUBROUTINE CONTROL C++   
!================================================
! INPUT  : champs inst u(x,y,t),v(x,y,t), IPASCONTROL

! (transformation en image PIV: SEB)
! TOURB:
! 1- localisation
! 2- extraction: position, rayon (ou vitesse max: u et v)
! 3- comparaison avec objectif
! 4- loi de control afin d'atteindre objectif: aspiration proportionnel à la distance et vitesse ? 
! (5- adaptation changement variable code DNS: MATTHIEU)

! OUTPUT : modification control u() and v() à la paroi en differentes position en x
